...
class BarChart extends Chart {
  ...
  add_scale() {
    ...
    let max = d3.max(this.data, (d) =>
      parseInt(d[this.y_field1].replaceAll(",", ""))
    );
    this.y = d3.scaleLinear().domain([0, max]).range([this.innerH, 0]);
    this.years = d3.groups(this.data, (d) => d.Year);
    this.x = d3
      .scaleBand()
      .domain(this.years.map((d) => d[0]))
      .range([0, this.innerW]);
  }
  update_data() {}

  draw_chart() {
    const getNumber = (d, field) =>
      parseInt(d[1][0][field].replaceAll(",", ""));

    let g = this.ChartArea.selectAll(".bar_g")
      .data(this.years)
      .join("g")
    ...
    g.append("rect")
      .attr("x", 10)
      .attr("y", (d) =>
        this.y(parseInt(d[1][0][this.y_field1].replaceAll(",", "")))
      )
    ...
    g.append("rect")
      ...
      .attr("y", (d) =>
        this.y(parseInt(d[1][0][this.y_field2].replaceAll(",", "")))
      )
      .attr("width", this.x.bandwidth() / 3)
      .attr(
        "height",
    ...
    g.append("text")
      ...
      .attr(
        "y",
        (d) => this.y(parseInt(d[1][0][this.y_field1].replaceAll(",", ""))) - 10
      )
...
class StackBar extends Chart {
  ...
  add_scale() {
    ...
    this.color = d3
      .scaleOrdinal()
      .domain(this.keys)
      .range(["#326D50", "#418E68", "#66AF60", "#BBECB7"]);
  }
  update_data() {
    // stack
  ...
  add_legends() {
    let g = this.svg.append("g").attr("transform", `translate(${10},${-40})`);
...
class Bar extends Chart {
  constructor(id, data) {
    ...
    this.AxisY.transition()
      .delay(200)
      .call(d3.axisLeft(this.y).tickFormat(d3.format(".1s")));
    this.add_legends();
  ...
  add_scale() {
    this.x = d3
      .scaleBand()
      .range([0, this.innerW])
      .domain(this.data.map((d) => d[this.x_field]));
    this.y = d3
      .scaleLinear()
      .range([this.innerH, 0])
      .domain([0, d3.max(this.data, (d) => +d[this.y_field])]);
    this.color = d3
      .scaleOrdinal()
      .domain(this.keys)
      .range(["#598DA4", "#68A3BC", "#79B0C7", "#B0E4FA"]);
  }
  update_data() {
  ...
  draw_chart() {
    let g = this.ChartArea.selectAll(".myg")
      .data(this.data)
      .join("g")
      .attr("class", "myg")
    ...
    let rects = g
      .selectAll("rect")
      .data((d) => Object.entries(d).filter((v) => v[0] !== this.x_field))
      .join("rect");
    rects
  ...
  add_legends() {
    let g = this.svg.append("g").attr("transform", `translate(${10},${-20})`);
...
d3.select("#y2019").on("click", () => {
  ...
  d3.select("#page4")
    ...
    .duration(1000)
    .ease(d3.easeBack)
    .style("grid-template-columns", "10% 10% 10% 50% 10% 10%");
...
d3.select("#y2016").on("click", () => {
  ...
  d3.select("#page4")
    ...
    .duration(1000)
    .ease(d3.easeBack)
    .style("grid-template-columns", "10% 10% 50% 10% 10% 10%");
...
d3.select("#y1980").on("click", () => {
  ...
  d3.select("#page4")
    ...
    .duration(1000)
    .ease(d3.easeBack)
    .style("grid-template-columns", "50% 10% 10% 10% 10% 10%");
...
let popIPdata = [
  ...
  {
    name: "TheMonsters",
    content: `Hong Kong born artist, Kasing, started to collaborate with How2work to publish Chinese illustration story books and collectible figure series in 2011. In 2015, he created “the Monsters” series’ story and launched the Vinyl figure series of “The Monsters” by How2work.`,
  },
  {
    name: "Dimoo",
    content: `The little boy Dimoo likes to travel in all mysterious dream worlds, 
    because he can meet many friends who grow up together during the journey. 
    The main line of the DIMOO WORLD series created by designer Ayan, 
    with the little boy DIMOO as the protagonist, presents a huge world view with a dreamy beauty, and also interprets the designer's understanding of life.`,
  },

  {
    name: "Skullpanda",
    content: `In 2014, XIONGMIAO founded the art studio "BEIZHAI", 
    and at the end of 2018, she began to create her own series of "SKULLPANDA", 
    and established the art toy studio " LAZYNORTH". SKULLPANDA’s works are filled with recognizable personal charisma.`,
  },
  {
    name: "Pucky",
    content: `Pucky’s name is derived from Shakespeare's elf character, 
    representing a combination of the real and dream worlds. Pucky's creation is like a colorful dream world, 
    endowed with darkness and spirituality. Most of her themes originate from exploring the world of nature.`,
  },
  {
    name: "SatyrRory",
    content: `Seulgie entered the American art toy circle in 2016, 
    she specializes in creating simple and cute creatures. Satyr Rory, 
    based on the half-sheep, half-human god "Satyr" in Greek mythology, was one of Seulgie's subversive creations in 2016.`,
  },
  {
    name: "BOBO&COCO",
    content: `BOBO is an introverted balloon, and COCO will be more active. 
    BOBO and COCO are always together. Although they have different personalities, they are very different. 
    comfortable state. The designer hopes to bring this comfortable and healing state to fans.`,
  },
  {
    name: "Instinctoy",
    content: `In 2005, Hiroto Ohkubo established INSTINCTOY, 
    an original art toy brand. In 2008, INSTINC-TOY became a manufacturer of original toys and began to manufacture 
    and sell original prod-ucts of its own brand, and also began to collaborate with many designers.`,
  },
  {
    name: "COOLABO",
    content: `COOLABO (Coolrain & LABO: COOLABO) is an art toy brand created by toy artist Coolrain and creative director LABO, 
    initially starting with the name of CoolrainLABO in Korean art toy industry. In order to collaborate with more emerging artists, 
    the brand name was officially changed to COOLABO in 2021.`,
  },
  {
    name: "KENNETHYOKI",
    title: "KENNETH & YOKI",
    content: `Yoyo Yeung, a new generation artist specializing in illustration and sculpture, 
    graduated from Camberwell College of Art, University of the Arts London, majoring in illustration. 
    Yoyo has a talent for drawing since she was a child, and has a personality that is faithful to her true nature.`,
  },
];
...
function goto_Image(e) {
  ...
  contentBox
    .append("div")
    .html(popIPdata.find((d) => d.name === name)?.content)
  ...
  let imgdiv = multiImgBox
    .selectAll("div")
    .data(popIPdata)
    .join("div")
    .attr("class", (d) => `p6${d.name}`);
  ...
  left.on("click", () => {
    // 跳转到第五页
    const { offsetTop } = document.getElementById("page5");
    window.scrollTo({
      top: offsetTop,
      behavior: "smooth",
    });
...
let p7data = [
  ...
  },
  {
    ...
    "inadequate effect", that is, when you don't get something, your heart is calm, but once you
    get
  ...
  },
  {
    ...
    prompting consumers to buy again until they win the lottery. Curiosity drives consumers to try
    it
    the first time, and desire to collect drives repeat purchases.`,
  },
  {
    title: "#4 Scarcity Effect",
    content: `  Businesses use "hunger marketing" to create a sense of scarcity. In consumer psychology, people
...
function initPage7() {
  ...
  let content = div
    .append("div")
    .attr("class", (d, i) => `contentText p7Content`)
    .html((d) => d.content);

  let titles = d3.select("#p7Titles");
  titles
    .selectAll("div")
    .data(p7data)
    .join("div")
    .html((d) => d.title)
    .attr("class", "p7title title");
  // 求出容器元素
  const p7Content = document.querySelector("#p7Content");
  const p7Titles = document.querySelector("#p7Titles");
  // const childHeight = Number.parseInt(p7Content.offsetHeight / 4);
  window.addEventListener("scroll", handleScroll);
  function handleScroll() {
    let max = -1;
    for (let index = 0; index < p7Content.children.length; index++) {
      const element = p7Content.children[index];
    ...
    for (let index = 0; index <= p7Content.children.length - 1; index++) {
      if (index <= max) {
        p7Titles.children[index].style.opacity = 1;
      } else {
        p7Titles.children[index].style.opacity = 0;
      }
    }
  }
  handleScroll();
...
d3.selectAll(".guessSymbol-select").on("click", (e) => {
  const target = e.target;
  const el = target.parentElement.previousElementSibling;
  el.textContent = target.innerText;
  const count = Number(target.parentElement.dataset.count);

  if (target.innerText.length === count) {
    d3.select(el).classed("error", false);
    d3.select(el).classed("right", true);
    target.parentElement.parentElement.nextElementSibling.style.visibility =
      "visible";
  } else {
    d3.select(el).classed("error", true);
    d3.select(el).classed("right", false);
    target.parentElement.parentElement.nextElementSibling.style.visibility =
      "hidden";
  }
});
